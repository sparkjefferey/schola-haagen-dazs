...
### 单点回归套件（自建数据、跑完自清，适合改完随手一跑）

```bash
npm run dev -p 3100            # 终端 1（注意：这几个套件都不要设 COOL_DOWN_HOURS=0）
npm run e2e:register          # 终端 2：注册表单（缺项逐栏点名、雅名查重、撞名流程、按钮不卡死）
npm run e2e:papers            # 终端 2：投稿门槛（冷静期、认证印越权、无附件投稿、附件校验）
npm run e2e:find-friend       # 终端 2：学友检索与申请（按学号/雅名找人 → 申请 → 红点 → 应允 → 无限私聊；
                              #          含管理者视角、申请栏下手处、输入法上屏 Enter 护栏）
```
...
- 四个套件都只写本地库，`BASE` 指向非本地地址时直接拒绝运行；测试账号用 `e2e*/papertest*` 前缀并在结束时清除。
- `e2e:papers` 需**默认 24 小时冷静期**才能覆盖冷静期用例，故不要按上面的 `COOL_DOWN_HOURS=0` 启动。
- `e2e:find-friend` 与 `e2e:ai` 都会在整轮中监听响应码，**任何非 2xx 都计为失败**（中文用户名曾在此暴露过一记 500，见 `docs/security/SECURITY_LOG.md` 2026-09-18 条）。
- `e2e:ai` 另有一道自检：若站点已配置模型、桩服务却一个请求都没收到，**立即中止**并提示——那说明 `AI_BASE_URL` 没指向桩服务，继续跑就会打到真服务上花钱。

## 会员与门派治理（本版新增）

- **自由入学 / 凭函就任**：学者可在 `/register` 自由注册；管理者必须凭掌门签发的邀请函（`R-xxxx`，见燕京阁·徽章司）宣誓就任，或由站长预设 `ADMIN_INVITE` 环境变量。
- **掌门认证**：新学者投稿先入「已收稿」，待掌门分诊；**认证学者免分诊，投稿直接进「送审中」**。无论是否认证，均须掌门走完审稿流程（录用→刊印）方可示人；稿件被刊印时自动为作者授予「认证学者」印。
- **入派冷静期**：新入派者须满 24 小时方可著书；**已授认证印者不受此限，授印后即刻可投稿**（可用环境变量 `COOL_DOWN_HOURS=0` 关闭）。论坛发言不受冷静期限制，新号仅限速（2 帖/小时）。
...
- **学术论坛（/forum）**：六大栏目、发帖/回帖、删帖（本人或管理者）、检举；
  回复里写 `@学正` 可由 AI（可选的 OpenAI 兼容模型，默认 DeepSeek）点评所引论著或就本帖讲解，
  同帖再召唤即追问（每人每日限额 + 全站日总闸；留空 `AI_API_KEY` 即整块关闭）
...
- **学者名册（/users/用户名）**：个人论著（含待审/被打回稿的本人处置：弃稿、改稿重投）、认证徽记、封籍状态、学号
- **站内讯息（/messages）**：**学友检索**（按学号 `#7` 或雅名/用户名找人，每 10 分钟 30 次）、
  **学友申请**（栏内即可检索找人发起；收到与发出的申请就地应允/婉拒，红点挂到回应为止）、
  私聊（未互证每日限额、互证后无限）、系统通知、论辩回应；管理者同样收发学友申请，
  另有一条「全员名录」便册与「学友名录」分列
