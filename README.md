...
- 三者都只写本地库，`BASE` 指向非本地地址时直接拒绝运行；测试账号用 `e2e*/papertest*` 前缀并在结束时清除。
- `e2e:papers` 需**默认 24 小时冷静期**才能覆盖冷静期用例，故不要按上面的 `COOL_DOWN_HOURS=0` 启动。
- `e2e:find-friend` 会在整轮中监听响应码，**任何非 2xx 都计为失败**（中文用户名曾在此暴露过一记 500，见 `docs/security/SECURITY_LOG.md` 2026-09-18 条）。
...
- **站内讯息（/messages）**：**学友检索**（按学号 `#7` 或雅名/用户名找人，每 10 分钟 30 次）、
  **学友申请**（互证申请就地应允/婉拒，红点挂到回应为止）、私聊（未互证每日限额、互证后无限）、
  系统通知、论辩回应
