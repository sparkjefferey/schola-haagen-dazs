...
tmp="$(mktemp)"
if [ -f .env ]; then
  grep -vE '^(AI_API_KEY|AI_BASE_URL|AI_MODEL|AI_DAILY_PER_USER|AI_DAILY_GLOBAL|AI_MAX_CONCURRENT|AI_MAX_OUTPUT_TOKENS|AI_REASONING_EFFORT|AI_MAX_INPUT_CHARS|AI_TIMEOUT_MS)=' .env > "$tmp" || true
fi
{
  cat "$tmp"
  echo ""
  ...
  echo "AI_MAX_OUTPUT_TOKENS=6000"
echo "AI_REASONING_EFFORT=none"
